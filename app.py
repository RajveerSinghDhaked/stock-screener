...
from db.connection import init_db as init_database, get_cursor
from data.fetcher import fetch_stock_history, fetch_weekly_data, fetch_monthly_data, fetch_stock_news
...
# ═══════════════════════════════════════════════════════════════
# INLINE SCAN (works on local + cloud)
# ═══════════════════════════════════════════════════════════════
...
def _load_or_fetch(symbol, tf):
    is_fresh_fn = {"daily": is_daily_fresh, "weekly": is_weekly_fresh, "monthly": is_monthly_fresh}[tf]
    store_fn = {"daily": store_daily_candles, "weekly": store_weekly_candles, "monthly": store_monthly_candles}[tf]
    get_fn = {"daily": get_daily_candles, "weekly": get_weekly_candles, "monthly": get_monthly_candles}[tf]
    fetch_fn = {"daily": fetch_stock_history, "weekly": fetch_weekly_data, "monthly": fetch_monthly_data}[tf]
    fetch_args = {"daily": ("1y", "1d"), "weekly": ("2y",), "monthly": ("5y",)}[tf]
...
def _save_scan_result(batch_time, symbol, score, regime, signals, analysis, indicators):
    comprehensive = {
        **analysis,
...
def run_full_scan(progress_callback=None):
    ...
    for i, symbol in enumerate(symbols):
        try:
            ...
            weekly_ind = compute_all_indicators(weekly_df) if weekly_df is not None and len(weekly_df) > 20 else None
            monthly_ind = compute_all_indicators(monthly_df) if monthly_df is not None and len(monthly_df) > 12 else None

            analysis = generate_veteran_analysis(symbol, score, regime, indicators, signals, weekly_ind, monthly_ind)
            _save_scan_result(batch_time, symbol, score, regime, signals, analysis, indicators)
    ...
    return load_fresh_scan_results()
...
with st.sidebar:
    ...
    if st.button("🔄 Refresh Scan", width="stretch", type="primary"):
        st.cache_data.clear()
        st.rerun()
...
if page == "🔍 Scanner":
    ...
    st.caption("Automated multi-timeframe analysis • Cached for 15 min • Works on cloud + local")

    tab_current, = st.tabs(["🔍 Current Scan"])

    with tab_current:
        results = get_cached_scan_results()
        if results:
